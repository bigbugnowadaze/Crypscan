"""P3.A acceptance gate — ChatGPT's full target sweep.

Acceptance targets (per the audit of PR #10 + freeze doc):
...
This sweeps each axis with multiple severities and records pass/fail. Geometric
sweep is wider than spike-8A's (full rotation, full scale range). Photometric
sweep matches spike-7's targets.
"""
...
P3A_DIR = Path(__file__).resolve().parent
sys.path.insert(0, str(P3A_DIR))
...
def make_payload() -> bytes:
    base = (
        "Phoxcar P3.A acceptance gate payload.\n"
        "ArUco fiducials + spike-7 substrate.\n"
    )
...
def main() -> int:
    ...
    if not sanity_ok:
        print("    ABORT: P3.A baseline failed before noise applied")
        return 1

    out_dir = P3A_DIR / 'results' / 'sweep_carriers'
    ...
    profile = {
        'timestamp': timestamp,
        'spike_version': 'P3A.0',
        'substrate': 'spike-7 codebook + pilots + ArUco fiducials @ 8-bit',
    ...
    out_json = P3A_DIR / 'results' / f'acceptance_profile_{timestamp}.json'
    out_json.write_text(json.dumps(profile, indent=2))
    print(f"\n  full profile -> {out_json.relative_to(P3A_DIR)}")
    return 0

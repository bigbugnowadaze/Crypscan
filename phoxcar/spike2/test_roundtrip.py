"""Phoxcar spike-2 acceptance gate (CRYPSOID-faithful).

Runs the full encode -> render (strict H^2) -> extract (LM nonlinear fit) ->
decode pathway on a synthetic ~1000-germ payload and asserts:

    1. SHA-256 of decoded == SHA-256 of input  (the GATE)
    2. Reed-Solomon ECC absorbs single-byte flips
    3. Total wall time < 60 s

Records gate results at a chosen pixel bit depth. The spike's primary
deliverable is the comparison across 8-bit and 16-bit (run both):

    python3 test_roundtrip.py --bit-depth 8
    python3 test_roundtrip.py --bit-depth 16

The spike-2 finding: CRYPSOID's strict H^2 evaluator pushes the per-germ
inverse fit harder than spike-1's linear model, and 8-bit pixel depth
exceeds RS(255, 223)'s 6.3% byte-error budget. 16-bit recovers cleanly.
"""
...
def make_payload(target_n_germs: int) -> bytes:
    ...
    Brotli ratio ~0.20 for varied text. RS adds ~14% overhead. Bit-pack
    is 39 bits / germ, i.e. 4.875 bytes / germ. So target source size:

        target_n_germs * 4.875 / 0.20 / 1.14 ~= target_n_germs * 21.4
    """
    base = (
        "Phoxoidal carrier spike-2 payload — Phase 0.5 acceptance gate.\n"
        "If this text decodes byte-exact via SHA-256 verification, the\n"
        "CRYPSOID-faithful symbol-decode pathway through 5-coefficient\n"
        "catastrophe germs (Pearcey-class basis, orthonormalized) is\n"
        "mathematically tractable at this pixel bit depth.\n"
    )
    rng = np.random.default_rng(seed=20260503)
    words = base.split()
    n_pad_words = max(int(target_n_germs * 21.4) // 7, 100)
...
def main(target_n_germs: int = 1000, pixel_bit_depth: int = 16) -> int:
    ...
    print("=" * 70)
    print(f"  phoxcar spike-2 — CRYPSOID-faithful acceptance gate")
    ...
    out_png = SPIKE_DIR / 'results' / f'spike2_carrier_{pixel_bit_depth}bit.png'
    out_png.parent.mkdir(parents=True, exist_ok=True)

    print()
    print(f"  ENCODING (CRYPSOID strict H^2 + orthonormal codec + RS) ...")
    ...
    print(f"    rs_encoded_size  : {enc_manifest['rs_encoded_size']:,} "
          f"({enc_manifest['rs_encoded_size']/enc_manifest['framed_size']*100:.1f}% of framed)")
    print(f"    encode wall time : {t_enc:.2f}s")

    print()
    print(f"  DECODING (LM trust-region fit + RS error correction) ...")
    ...
    if res is None:
        ...
        report = {
            'timestamp': timestamp, 'spike_version': '2.0',
    ...
    print(f"    sign flips applied: {res.sign_flips_applied}/{res.n_germs}")
    ...
    print(f"  GATE 3 — total wall time < 60 s       : {'PASS' if time_ok else 'FAIL'} "
          f"({total_time:.2f}s)")
    print()
    print(f"  OVERALL                              : "
    ...
    report = {
        ...
        'sign_flips_applied': int(res.sign_flips_applied),

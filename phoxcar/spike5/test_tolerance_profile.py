"""Spike-5 tolerance profile — re-runs the spike-4 sweep against the
R=3 + RS(255, 191) substrate.

For direct comparison: same payload size target, same noise types, same
severities. Difference is only in the substrate's mitigation stack.
"""
...
SPIKE5_DIR = Path(__file__).resolve().parent
sys.path.insert(0, str(SPIKE5_DIR))
...
def make_payload(target_n_germs_per_copy: int = 200) -> bytes:
    """Smaller payload than spike-3's gate to keep the sweep fast at R=3.

    With R=3 and ~14% RS overhead, target_n_germs_per_copy * 5 / 0.20 / 1.14
    bytes of source text gives roughly target_n_germs_per_copy germs per
    replica copy, total ~3 * target_n_germs_per_copy in the carrier.
    """
    base = (
        "Phoxcar spike-5 tolerance profile payload. R=3 + RS(255, 191).\n"
        "Same noise sweep as spike-4 for direct comparison.\n"
    )
    rng = np.random.default_rng(seed=20260504)
    words = base.split()
    n_pad_words = max(int(target_n_germs_per_copy * 21.9) // 7, 100)
...
def run_one_condition(canvas, noise_type, noise_fn, severity,
    ...
    return {
        ...
        'decode_error': None,
        'vote_unanimous': res.vote_unanimous,
        'vote_majority': res.vote_majority,
        'vote_tied': res.vote_tied,
...
def summarize(results):
    ...
    for nt, rs in by_type.items():
        ...
        if nt == 'jpeg_roundtrip':
            ...
            metric = "JPEG quality (lower is more severe)"
            ...
            metric = "severity (higher is more severe)"
        summary.append({
            'noise_type': nt,
...
def main(target_n_germs_per_copy: int = 200) -> int:
    print("=" * 70)
    print("  phoxcar spike-5 — R=3 + RS(255, 191) tolerance profile")
    print("=" * 70)

    payload = make_payload(target_n_germs_per_copy)
    expected_sha = hashlib.sha256(payload).digest()
    base_png = SPIKE5_DIR / 'results' / 'spike5_base_carrier.png'
    base_png.parent.mkdir(parents=True, exist_ok=True)
    print(f"\n  payload size       : {len(payload):,}")
    print(f"  expected sha256    : {expected_sha.hex()[:32]}...")

    print(f"\n  ENCODING (R=3, RS(255, 191), 8-bit) ...")
    t0 = time.perf_counter()
    enc_manifest = encode(
        payload, "payload.txt", base_png,
        params=EncodeParams(pixel_bit_depth=8, R=3),
        sidecar=True,
    )
    ...
    print(f"    replicated_size  : {enc_manifest['replicated_size']:,} (R={enc_manifest['params']['R']})")
    ...
    print(f"    vote stats       : unanimous={sanity_res.vote_unanimous}, "
          f"majority={sanity_res.vote_majority}, tied={sanity_res.vote_tied}")
    print(f"    RS               : corrected={sanity_res.rs_corrected_frames} frames, "
          f"failed={len(sanity_res.rs_failed_frames)}")
    if not sanity_ok:
        print("    ABORT: spike-5 baseline failed before noise applied")
        return 1

    out_dir = SPIKE5_DIR / 'results' / 'noisy_carriers'
    ...
    for noise_type, noise_fn, severities in NOISE_SWEEP:
        for sev in severities:
            ...
            extra = ''
            if 'vote_tied' in res:
                extra = (f"  vote U={res['vote_unanimous']} M={res['vote_majority']} "
                         f"T={res['vote_tied']}  RS_corr={res['rs_frames_corrected']} "
                         f"RS_fail={res['rs_frames_failed']}")
    ...
    profile = {
        'timestamp': timestamp,
        'spike_version': '5.0',
        'substrate': 'spike-3 sigmoid + R=3 + RS(255, 191) @ 8-bit',
    ...
    out_json = SPIKE5_DIR / 'results' / f'tolerance_profile_{timestamp}.json'
    out_json.write_text(json.dumps(profile, indent=2))
    print(f"\n  full profile -> {out_json.relative_to(SPIKE5_DIR)}")
    return 0

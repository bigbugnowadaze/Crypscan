"""P3.B decoder: phoxoidal-native pose recovery + spike-7 pipeline.

NO SIDECAR. NO CONVENTIONAL CV. Detection uses ONLY the catastrophe-germ
basis itself (matched-filter NCC against germ templates).

Pipeline:
    PNG carrier (potentially warped, noisy, off-axis)
      -> read 8-bit grayscale
      -> NCC matched-filter detection of 4 corner clusters
      -> 4-point homography (observed corner centers -> canonical 1280x1280)
      -> rectify
...
class DecodeResult:
    ...
    pose_ok: bool
    pose_scores: list      # [(corner_name, ncc_score, scale), ...]
...
def _empty(error: str, pose_ok: bool = False, pose_scores=None) -> DecodeResult:
    return DecodeResult(
        ...
        pose_ok=pose_ok, pose_scores=pose_scores or [],
        decode_error=error,
    )
...
def decode(png_path: Path, params: EncodeParams | None = None) -> DecodeResult:
    ...
    # 1. Phoxoidal-native pose recovery
    pose = detect_pose(captured, basis, layout)
    pose_scores = [(d.name, d.peak_score, d.best_scale) for d in pose.detections]
    if not pose.success:
        return _empty(f"pose failed: {pose.error}",
                       pose_ok=False, pose_scores=pose_scores)

    # 2. Rectify to canonical frame
    rectified = rectify(captured, pose.homography, CANVAS_WIDTH, CANVAS_HEIGHT)

    # 3. Pilot fit FIRST (per P3.A's pilots-before-manifest finding)
    ...
    try:
        ...
        return _empty(f"pilot fit failed: {e}",
                       pose_ok=True, pose_scores=pose_scores)

    rectified_corrected = transform.invert(rectified).astype(np.float32)

    # 4. Manifest cluster on the corrected image
    ...
    try:
        ...
        return _empty(f"manifest fit failed: {e}",
                       pose_ok=True, pose_scores=pose_scores)
    ...
    try:
        ...
        return _empty(f"manifest parse failed: {e}",
                       pose_ok=True, pose_scores=pose_scores)
    ...
    try:
        ...
        return _empty(f"final decode failed: {e}",
                       pose_ok=True, pose_scores=pose_scores)
